                ...
                <div class="box-body">
                    <form role="form" action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-8">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group {{ $errors->first('c_menu_id') ? 'has-error' : '' }}">
                                    <label class="control-label">Chọn menu <b class="text-danger">(*)</b></label>
                                    <select name="c_menu_id" class="form-control ">
                                        <option value="">__Click__</option>
                                        @foreach($menus as $menu)
                                            <option value="{{ $menu->id }}" {{ (old('c_menu_id', isset($category->c_menu_id) ? $category->c_menu_id : '') == $menu->id) ? "selected='selected'" : "" }}>
                                                {{  $menu->mn_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @if ($errors->first('c_menu_id'))
                                        <span class="text-danger">{{ $errors->first('c_menu_id') }}</span>
                                    @endif
                                </div>
                            </div>
                            <div class="col-sm-12">
