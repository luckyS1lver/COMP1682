        ...
        if (!empty($categoriesHot)) {
            foreach ($categoriesHot as $key => $cate) {
                if ($cate['c_parent_id'] == 0) {
                    $cateCh = Category::where('c_parent_id', $cate['id'])->pluck('id')->toArray();
                    $listCate = array_merge($cateCh, [$cate['id']]);
                    $product = Product::whereIn('pro_category_id', $listCate)->orderByDesc('id')->get();
                } else {
